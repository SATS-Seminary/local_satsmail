...
/**
 * Search parameters for querying messages.
 *
...
class message_search {
    ...
    private function get_base_sql(bool $countperlabel = false) {
        ...
        if ($this->label || $countperlabel) {
            $fromsql = 'FROM {local_satsmail_message_labels} i';
            $conditions['labelid'] = $this->label->id ?? array_keys(label::get_by_user($this->user));
        } else {
            $fromsql = 'FROM {local_satsmail_message_users} i';
        ...
        if ($this->content != '' || $this->withfilesonly) {
            $fromsql .= ' JOIN {local_satsmail_messages} m ON m.id = i.messageid';
        }

        if ($this->content != '') {
            $fullnamefield = $DB->sql_fullname('u.firstname', 'u.lastname');
            $usersql = "SELECT mu.messageid FROM {local_satsmail_message_users} mu"
        ...
        if ($this->startid) {
            $starttime = $DB->get_field('local_satsmail_messages', 'time', ['id' => $this->startid]);
            if ($starttime === false) {
        ...
        if ($this->stopid) {
            $stoptime = $DB->get_field('local_satsmail_messages', 'time', ['id' => $this->stopid]);
            if ($stoptime === false) {
