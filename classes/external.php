...
class external extends \external_api {
    ...
    public static function get_preferences_raw() {
        $result = [
            ...
            'markasread' => (bool) get_user_preferences('local_satsmail_markasread', 0),
            'notifications' => [],
        ];

        if (!get_config('message', 'local_satsmail_mail_disable')) {
            $configenabled = get_config('message', 'message_provider_local_satsmail_mail_enabled');
            $prefenabled = get_user_preferences('message_provider_local_satsmail_mail_enabled');
    ...
    public static function set_preferences() {
        ...
        if (isset($params['preferences']['perpage'])) {
            set_user_preference('local_satsmail_mailsperpage', $params['preferences']['perpage']);
        }

        if (isset($params['preferences']['markasread'])) {
            set_user_preference('local_satsmail_markasread', $params['preferences']['markasread']);
        }
        ...
        if (isset($params['preferences']['notifications'])) {
            ...
            set_user_preference('message_provider_local_satsmail_mail_enabled', $enabled);
        }
    ...
    public static function get_message_response(user $user, message $message) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'local_satsmail', 'message', $message->id, 'filename', false);
        ...
        foreach ($message->get_references() as $ref) {
            $attachments = [];
            $files = $fs->get_area_files($context->id, 'local_satsmail', 'message', $ref->id, 'filename', false);
