...
/**
 * Label defined by a user to classify messages.
 */
class label {
     ...
     *
     * @param \stdClass $record A database record from table local_satsmail_labels.
     */
    ...
    public static function cache(): \cache {
        return \cache::make('local_satsmail', 'labels');
    }
    ...
    public static function create(user $user, string $name, string $color = ''): self {
        ...
        $record->color = $color;
        $record->id = $DB->insert_record('local_satsmail_labels', $record);
    ...
    public static function get_by_user(user $user): array {
        ...
        $labels = [];
        foreach ($DB->get_records('local_satsmail_labels', ['userid' => $user->id]) as $record) {
    ...
    public static function get_many(array $ids): array {
        ...
        if ($missingids) {
            ...
            $records = $DB->get_records_select('local_satsmail_labels', "id $sqlid", $params);
    ...
    public function delete(): void {
        ...
        $DB->delete_records('local_satsmail_labels', ['id' => $this->id]);
        $DB->delete_records('local_satsmail_message_labels', ['labelid' => $this->id]);
    ...
    public function update(string $name, string $color): void {
        ...
        $DB->update_record('local_satsmail_labels', $record);
