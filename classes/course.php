...
class course {
    ...
    public static function cache(): \cache {
        return \cache::make('local_satsmail', 'courses');
    }
    ...
    public static function get_by_user(user $user): array {
        ...
        foreach (enrol_get_users_courses($user->id, true) as $record) {
            ...
            if (has_capability('local/satsmail:usemail', $context, $user->id, false)) {
    ...
    public function get_viewable_roles(user $user): array {
        $result = [];
        [$needed, $forbidden] = get_roles_with_cap_in_context($this->get_context(), 'local/satsmail:usemail');
