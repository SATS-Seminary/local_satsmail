...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('local_satsmail_labels', [
            ...
            'name' => 'privacy:metadata:local_satsmail_labels:name',
            'color' => 'privacy:metadata:local_satsmail_labels:color',
        ], 'privacy:metadata:local_satsmail_labels');

        $collection->add_database_table('local_satsmail_messages', [
            'courseid' => 'privacy:metadata:local_satsmail_messages:courseid',
            'subject' => 'privacy:metadata:local_satsmail_messages:subject',
            'content' => 'privacy:metadata:local_satsmail_messages:content',
            'format' => 'privacy:metadata:local_satsmail_messages:format',
            'attachments' => 'privacy:metadata:local_satsmail_messages:attachments',
            'draft' => 'privacy:metadata:local_satsmail_messages:draft',
            'time' => 'privacy:metadata:local_satsmail_messages:time',
            'normalizedsubject' => 'privacy:metadata:local_satsmail_messages:normalizedsubject',
            'normalizedcontent' => 'privacy:metadata:local_satsmail_messages:normalizedcontent',
        ], 'privacy:metadata:local_satsmail_messages');

        $collection->add_database_table('local_satsmail_message_labels', [
            'messageid' => 'privacy:metadata:local_satsmail_message_labels:messageid',
            'courseid' => 'privacy:metadata:local_satsmail_message_labels:courseid',
            'draft' => 'privacy:metadata:local_satsmail_message_labels:draft',
            'time' => 'privacy:metadata:local_satsmail_message_labels:time',
            'labelid' => 'privacy:metadata:local_satsmail_message_labes:labelid',
            'role' => 'privacy:metadata:local_satsmail_message_labels:role',
        ...
        $collection->add_database_table('local_satsmail_message_refs', [
            'messageid' => 'privacy:metadata:local_satsmail_message_refs:messageid',
            'reference' => 'privacy:metadata:local_satsmail_message_refs:reference',
        ], 'privacy:metadata:local_satsmail_message_refs');

        $collection->add_database_table('local_satsmail_message_users', [
            'messageid' => 'privacy:metadata:local_satsmail_message_users:messageid',
            'courseid' => 'privacy:metadata:local_satsmail_message_users:courseid',
            'draft' => 'privacy:metadata:local_satsmail_message_users:draft',
            'time' => 'privacy:metadata:local_satsmail_message_users:time',
            'userid' => 'privacy:metadata:local_satsmail_message_user:userid',
            'role' => 'privacy:metadata:local_satsmail_message_users:role',
        ...
        $collection->add_user_preference('local_satsmail_mailsperpage', 'privacy:metadata:preference:local_satsmail_mailsperpage');
        $collection->add_user_preference('local_satsmail_markasread', 'privacy:metadata:preference:local_satsmail_markasread');
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        if ($DB->record_exists('local_satsmail_labels', ['userid' => $userid])) {
    ...
    public static function export_user_data(approved_contextlist $contextlist): void {
        ...
        foreach (label::get_by_user($user) as $label) {
            ...
            $data = [
                'name' => $label->name,
                'color' => get_string('color' . $label->color, 'local_satsmail'),
            ];
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            foreach (self::iterate_messages($user, $context->instanceid) as $message) {
                ...
                $writer->export_area_files($subcontext, 'local_satsmail', 'message', $message->id);
            }
        }
    }
    ...
    public static function export_user_preferences(int $userid): void {
        ...
        if ($perpage !== null) {
            writer::export_user_preference(
                'local_satsmail',
                'local_satsmail_mailsperpage',
                $perpage,
                strings::get('privacy:metadata:preference:local_satsmail_mailsperpage'),
            );
        }

        $markasread = get_user_preferences('local_satsmail_markasread', null, $userid);
        if ($markasread !== null) {
            writer::export_user_preference(
                'local_satsmail',
                'local_satsmail_markasread',
                transform::yesno($markasread),
                strings::get('privacy:metadata:preference:local_satsmail_markasread'),
            );
        }
    }
    ...
    public static function delete_data_for_users(approved_userlist $userlist): void {
        foreach ($userlist->get_users() as $user) {
            ...
            self::delete_data_for_user(new approved_contextlist($user, 'local_satsmail', [$contextid]));
        }
    }
    ...
    private static function iterate_messages(user $user, int $courseid): \Traversable {
        ...
        . ' FROM {local_satsmail_message_users}'
        . ' WHERE userid = :userid'
