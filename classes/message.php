...
class message {
     ...
     *
     * @param \stdClass $record Record of local_satsmail_messages.
    ...
    public static function create(message_data $data): self {
        ...
        $messagerecord->id = $DB->insert_record('local_satsmail_messages', $messagerecord);
        ...
        // References.
        if ($data->reference) {
            $records = $DB->get_records('local_satsmail_message_refs', ['messageid' => $data->reference->id]);
            foreach ($records as $record) {
                unset($record->id);
                $record->messageid = $message->id;
            }
            $records[] = (object) ['messageid' => $message->id, 'reference' => $data->reference->id];
            $DB->insert_records('local_satsmail_message_refs', $records);
        }
    ...
    public static function delete_course_data(\context_course $context): void {
        ...
        $DB->delete_records('local_satsmail_message_labels', ['courseid' => $context->instanceid]);

        $DB->delete_records('local_satsmail_message_users', ['courseid' => $context->instanceid]);

        $select = 'messageid IN (SELECT id FROM {local_satsmail_messages} WHERE courseid = :courseid)';
        $DB->delete_records_select('local_satsmail_message_refs', $select, ['courseid' => $context->instanceid]);

        $DB->delete_records('local_satsmail_messages', ['courseid' => $context->instanceid]);

        $transaction->allow_commit();

        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'local_satsmail');
    }
    ...
    public static function get_many(array $ids): array {
        ...
        $messagerecords = $DB->get_records_select('local_satsmail_messages', "id $sqlid", $params, '', $fields);
        ...
        $sql = 'SELECT ml.id, ml.messageid, ml.labelid, l.userid'
            . ' FROM {local_satsmail_message_labels} ml'
            . ' JOIN {local_satsmail_labels} l ON l.id = ml.labelid'
    ...
    public function get_references(bool $forward = false, int $offset = 0, int $limit = 0): array {
        ...
        $sql = 'SELECT m.id'
            . ' FROM {local_satsmail_message_refs} mr'
            . " JOIN {local_satsmail_messages} m ON m.id = mr.$otherfield"
            . " WHERE mr.$thisfield = ?"
    ...
    public function send(int $time): void {
        ...
        $DB->set_field('local_satsmail_messages', 'draft', 0, ['id' => $this->id]);
        $DB->set_field('local_satsmail_messages', 'time', $time, ['id' => $this->id]);
        $DB->set_field('local_satsmail_message_users', 'draft', 0, ['messageid' => $this->id]);
        $DB->set_field('local_satsmail_message_users', 'time', $time, ['messageid' => $this->id]);
        $DB->set_field('local_satsmail_message_labels', 'draft', 0, ['messageid' => $this->id]);
        $DB->set_field('local_satsmail_message_labels', 'time', $time, ['messageid' => $this->id]);
    ...
    public function set_deleted(user $user, int $status): void {
        ...
        if ($fulldelete) {
            $DB->delete_records('local_satsmail_messages', ['id' => $this->id]);
            $DB->delete_records('local_satsmail_message_refs', ['messageid' => $this->id]);
            $DB->delete_records('local_satsmail_message_users', ['messageid' => $this->id]);
            $DB->delete_records('local_satsmail_message_labels', ['messageid' => $this->id]);
        } else {
            ...
            if ($resetcontent) {
                ...
                $DB->update_record('local_satsmail_messages', $data);
            }
            ...
            if ($resetmetadata) {
                ...
                $DB->set_field('local_satsmail_message_users', 'deleted', $status, $conditions);
            }
            ...
            if ($this->labelids[$user->id]) {
                ...
                if ($resetmetadata) {
                    $DB->delete_records_select('local_satsmail_message_labels', $select, $params);
                } else {
                    $DB->set_field_select('local_satsmail_message_labels', 'deleted', $status, $select, $params);
                }
            }
        }
        ...
        if ($resetcontent && $this->attachments > 0) {
            ...
            $fs->delete_area_files($context->id, 'local_satsmail', 'message', $this->id);
        }
    ...
    public function set_labels(user $user, array $labels): void {
        ...
        foreach ($this->labelids[$user->id] as $labelid) {
            if (!in_array($labelid, $labelids)) {
                $DB->delete_records('local_satsmail_message_labels', ['messageid' => $this->id, 'labelid' => $labelid]);
            }
        }
    ...
    public function set_starred(user $user, bool $status): void {
        ...
        $conditions = ['messageid' => $this->id, 'userid' => $user->id];
        $DB->set_field('local_satsmail_message_users', 'starred', $status, $conditions);

        foreach ($this->labelids[$user->id] as $labelid) {
            $conditions = ['messageid' => $this->id, 'labelid' => $labelid];
            $DB->set_field('local_satsmail_message_labels', 'starred', $status, $conditions);
        }
    ...
    public function set_unread(user $user, bool $status): void {
        ...
        $conditions = ['messageid' => $this->id, 'userid' => $user->id];
        $DB->set_field('local_satsmail_message_users', 'unread', $status, $conditions);

        foreach ($this->labelids[$user->id] as $labelid) {
            $conditions = ['messageid' => $this->id, 'labelid' => $labelid];
            $DB->set_field('local_satsmail_message_labels', 'unread', $status, $conditions);
        }
    ...
    public function update(message_data $data): void {
        ...
        // Course.
        if (!$DB->record_exists('local_satsmail_message_refs', ['messageid' => $this->id])) {
        ...
        $this->attachments = count($fs->get_area_files($newcontext->id, 'local_satsmail', 'message', $this->id, '', false));
        ...
        $DB->update_record('local_satsmail_messages', $messagerecord);
        ...
        foreach (array_keys($this->roles) as $userid) {
            if ($this->roles[$userid] != self::ROLE_FROM && empty($ignored[$userid])) {
                ...
                $DB->delete_records('local_satsmail_message_users', ['messageid' => $this->id, 'userid' => $userid]);
            }
        }

        // Delete old files.
        if ($newcontext->id != $oldcontext->id) {
            $fs->delete_area_files($oldcontext->id, 'local_satsmail', 'message', $this->id);
        }
