...
use local_satsmail\message_search;
...
abstract class testcase extends \advanced_testcase {
    ...
    protected static function assert_string_exists(string $identifier): void {
        self::assertTrue(
            get_string_manager()->string_exists($identifier, 'local_satsmail'),
    ...
    protected static function assert_attachments(array $expected, message $message) {
        ...
        $files = $fs->get_area_files($contextid, 'local_satsmail', 'message', $message->id, 'id', false);
        $actual = [];
     ...
     * @param string $table Table name without the "local_satsmail_" prefix.
    ...
    protected static function assert_record_count(int $expected, string $table, array $conditions = []) {
        ...
        $actual = $DB->count_records('local_satsmail_' . $table, $conditions);
    ...
    protected static function assert_record_data($table, array $conditions, array $data): void {
        ...
        $records = $DB->get_records('local_satsmail_' . $table, $conditions);
    ...
    public static function generate_random_data(bool $withmessages): array {
        ...
        // Generate roles.
        foreach (get_roles_with_capability('local/satsmail:usemail') as $role) {
            unassign_capability('local/satsmail:usemail', $role->id);
            unassign_capability('local/satsmail:mailsamerole', $role->id);
        }
        $rolecaps = [
            [],
            ['local/satsmail:usemail' => 'allow'],
            ['local/satsmail:usemail' => 'allow', 'local/satsmail:mailsamerole' => 'allow'],
        ];
