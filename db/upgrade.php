...
function xmldb_local_satsmail_upgrade($oldversion) {
    ...
    if ($oldversion < 2015121400) {
        // Clean obsolete local_satsmail_fullmessage preference.
    ...
    if ($oldversion < 2016070100) {
        // Define field attachments to be added to local_satsmail_messages.
        $table = new xmldb_table('local_satsmail_messages');
    ...
    if ($oldversion < 2016070101) {
        ...
        $sql = 'SELECT f.itemid, COUNT(*) as numfiles
                ...
                GROUP BY f.itemid';
        $params = [
            'component' => 'local_satsmail',
            'filearea' => 'message',
            'filename' => '.',
        ];
        $rs = $DB->get_recordset_sql($sql, $params);
        foreach ($rs as $record) {
            $DB->set_field('local_satsmail_messages', 'attachments', $record->numfiles, ['id' => $record->itemid]);
        }
        $rs->close();
    ...
    if ($oldversion < 2016070103) {
        ...
        unset_config('cronenabled', 'local_satsmail');
        unset_config('cronstart', 'local_satsmail');
    ...
    if ($oldversion < 2017070400) {
        // Define field normalizedsubject to be added to local_satsmail_messages.
        $table = new xmldb_table('local_satsmail_messages');
    ...
    if ($oldversion < 2017070401) {
        // Define field normalizedcontent to be added to local_satsmail_messages.
        $table = new xmldb_table('local_satsmail_messages');
    ...
    if ($oldversion < 2017070402) {
        ...
        while (true) {
            ...
            $records = $DB->get_records_select('local_satsmail_messages', $select, $params, 'id', $fields, 0, 100);
            foreach ($records as $record) {
                $data = new \stdClass();
                $data->id = $record->id;
                $data->normalizedsubject = \local_satsmail\message::normalize_text($record->subject, FORMAT_PLAIN);
                $data->normalizedcontent = \local_satsmail\message::normalize_text($record->content, $record->format);
    ...
    // Change type of field role on table local_satsmail_message_users to integer.
    ...
    if ($oldversion < 2023060700) {
        ...
        $DB->set_field('local_satsmail_message_users', 'role', 1, ['tmp_role' => 'from']);
        $DB->set_field('local_satsmail_message_users', 'role', 2, ['tmp_role' => 'to']);
        $DB->set_field('local_satsmail_message_users', 'role', 3, ['tmp_role' => 'cc']);
        $DB->set_field('local_satsmail_message_users', 'role', 4, ['tmp_role' => 'bcc']);
    ...
    if ($oldversion < 2023060702) {
        ...
        // Copy data from local_satsmail_messages.
        $fromsql = 'FROM {local_satsmail_messages} m WHERE m.id = mu.messageid';
        $sql = 'UPDATE {local_satsmail_message_users} mu SET'
    ...
    if ($oldversion < 2023060704) {
        ...
        // Copy courseid, draft and time from local_satsmail_messages.
        $fromsql = 'FROM {local_satsmail_messages} m WHERE m.id = ml.messageid';
        $sql = 'UPDATE {local_satsmail_message_labels} ml SET'
            . " courseid = (SELECT m.courseid $fromsql),"
            . " draft = (SELECT m.draft $fromsql),"
            . " time = (SELECT m.time $fromsql)";
        $DB->execute($sql);

        // Copy role, unread, starred and deleted from local_satsmail_message_users.
        $fromsql = 'FROM {local_satsmail_message_users} mu'
            . ' JOIN {local_satsmail_labels} l ON l.userid = mu.userid'
    ...
    if ($oldversion < 2023092200) {
        $table = new xmldb_table('local_satsmail_index');
    ...
    if ($oldversion < 2024030500) {
        $sql = 'SELECT r.id
                FROM {local_satsmail_message_refs} r
                JOIN {local_satsmail_messages} m1 ON m1.id = r.messageid
                JOIN {local_satsmail_messages} m2 ON m2.id = r.reference
                WHERE m1.courseid != m2.courseid';
        $rs = $DB->get_recordset_sql($sql);
        foreach ($rs as $record) {
            $DB->delete_records('local_satsmail_message_refs', ['id' => $record->id]);
        }
        $rs->close();
    ...
    if ($oldversion < 2024031400) {
        $processors = explode(',', get_config('message', 'message_provider_local_satsmail_mail_enabled') ?? '');
    ...
    if ($oldversion < 2026020500) {
        ...
        // Create field archived with default value 0.
    ...
    // Add archived field to local_satsmail_message_labels.
    ...
    if ($oldversion < 2026020502) {
        // Update index on local_satsmail_message_users.
        $table = new xmldb_table('local_satsmail_message_users');
        $index = new xmldb_index(
            'userid',
            XMLDB_INDEX_NOTUNIQUE,
        ...
        // Update index on local_satsmail_message_labels.
        $table = new xmldb_table('local_satsmail_message_labels');
        $index = new xmldb_index(
            'labelid',
            XMLDB_INDEX_NOTUNIQUE,
