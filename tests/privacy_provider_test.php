...
/**
 * Tests for the privacy provider functionality.
 *
...
use local_satsmail\privacy\provider;
use local_satsmail\output\strings;

/**
 * Tests for the privacy API implementation.
 *
 * @covers \local_satsmail\privacy\provider
 */
final class privacy_provider_test extends test\testcase {
    public function test_get_metadata(): void {
        ...
        $collection = new collection('local_satsmail');
        ...
        self::assertEqualsCanonicalizing(['local_satsmail_mailsperpage', 'local_satsmail_markasread'], $preferences);
    ...
    public function test_get_contexts_for_userid(): void {
        ...
        foreach ($users as $user) {
            ...
            provider::delete_data_for_user(
                new approved_contextlist(\core_user::get_user($user->id), 'local_satsmail', $expectedcontextids)
            );
    ...
    public function test_get_users_in_context(): void {
        ...
        foreach (get_courses() as $course) {
            ...
            $userlist = new userlist($context, 'local_satsmail');
            ...
            provider::delete_data_for_users(
                new approved_userlist($context, 'local_satsmail', array_keys($courseusers))
            );
            $userlist = new userlist($context, 'local_satsmail');
        ...
        $userlist = new userlist(\context_system::instance(), 'local_satsmail');
        provider::get_users_in_context($userlist);
    ...
    public function test_export_user_preferences(): void {
        ...
        set_user_preference('local_satsmail_mailsperpage', 20, $user);
        set_user_preference('local_satsmail_markasread', true, $user);

        provider::export_user_preferences($user->id);

        $this->assertTrue($writer->has_any_data());

        $preferences = (array) $writer->get_user_preferences('local_satsmail');

        $this->assertEquals((object) [
            'value' => 20,
            'description' => get_string('privacy:metadata:preference:local_satsmail_mailsperpage', 'local_satsmail'),
        ], $preferences['local_satsmail_mailsperpage']);

        $this->assertEquals((object) [
            'value' => get_string('yes'),
            'description' => get_string('privacy:metadata:preference:local_satsmail_markasread', 'local_satsmail'),
        ], $preferences['local_satsmail_markasread']);
    }
    ...
    public function test_delete_data_for_users(): void {
        ...
        $context = $course->get_context();
        $user = $messages[0]->sender();
        $userlist = new approved_userlist($context, 'local_satsmail', [$user->id]);
