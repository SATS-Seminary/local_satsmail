...
/**
 * Tests for the user_search class.
 *
 * @covers \local_satsmail\user_search
 */
final class user_search_test extends test\testcase {
    ...
    protected static function filter_users(array $users, user_search $search): array {
        ...
        $excludedroleids = [];
        if (!has_capability('local/satsmail:mailsamerole', $context, $search->user->id, false)) {
        ...
        foreach ($users as $user) {
            if (
                ...
                $user->deleted ||
                !is_enrolled($context, $user, 'local/satsmail:usemail', true) ||
