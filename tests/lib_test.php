...
final class lib_test extends test\testcase {
    public function test_pluginfile(): void {
        ...
        $result = local_satsmail_pluginfile(null, null, $course->get_context(), 'message', [$message1->id, 'file1.txt'], null);
        self::assertInstanceOf('\stored_file', $result);
        self::assertEquals($course->get_context()->id, $result->get_contextid());
        self::assertEquals('local_satsmail', $result->get_component());
        ...
        self::setUser($user2->id);
        $result = local_satsmail_pluginfile(null, null, $course->get_context(), 'message', [$message3->id, 'file3.txt'], null);
        self::assertFalse($result);

        // Inexistent message.
        self::setUser($user2->id);
        $result = local_satsmail_pluginfile(null, null, $course->get_context(), 'message', [$message1->id, 'file2.txt'], null);
        self::assertFalse($result);

        // Inexistent message.
        self::setUser($user2->id);
        $result = local_satsmail_pluginfile(null, null, $course->get_context(), 'message', [-1, 'file1.txt'], null);
        self::assertFalse($result);
    ...
    public function test_render_navbar_output(): void {
        ...
        $PAGE->set_url(new \moodle_url('/local/satsmail/view.php'));
