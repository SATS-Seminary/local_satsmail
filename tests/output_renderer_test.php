...
/**
 * Tests for the output renderer functionality.
 *
 ...
 *
 * @covers \local_satsmail\output\renderer
 */
final class output_renderer_test extends test\testcase {
    ...
    public function test_formatted_message_content(): void {
        ...
        $fileurl = new \moodle_url("/pluginfile.php/$context->id/local_satsmail/message/$message->id/file.txt");
    ...
    public function test_notification(): void {
        ...
        self::assertEquals('local_satsmail', $notification->component);
        ...
        $contexturl = new \moodle_url('/local/satsmail/view.php', ['t' => 'inbox', 'm' => $message->id]);
    ...
    public function test_svelte_script(): void {
        ...
        $url = preg_quote($CFG->wwwroot . '/local/satsmail/svelte/build/', '/') . 'view-[\w-]+\.js';
        $pattern = '/^<script type="module" src="' . $url . '"><\/script>$/';
        self::assertMatchesRegularExpression($pattern, $html);

        $url = preg_quote($CFG->wwwroot . '/local/satsmail/svelte/build/', '/') . 'view-[\w-]+\.css';
        ...
        $jsurl = preg_quote($CFG->wwwroot . '/local/satsmail/svelte/build/', '/') . 'view-[\w-]+\.js';
        $cssurl = preg_quote($CFG->wwwroot . '/local/satsmail/svelte/build/', '/') . 'view-[\w-]+\.css';
        ...
        set_config('local_satsmail_devserver', 'http://localhost:5173');
