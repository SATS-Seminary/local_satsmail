...
/**
 * Tests for the upgrade code.
 *
 * @covers \xmldb_local_satsmail_upgrade
 */
final class upgrade_test extends test\testcase {
    public function setUp(): void {
        ...
        require_once("$CFG->dirroot/local/satsmail/db/upgrade.php");
    }
    ...
    public function test_upgrade(): void {
        ...
        $dbman->delete_tables_from_xmldb_file("$CFG->dirroot/local/satsmail/db/install.xml");
        $dbman->install_from_xmldb_file("$CFG->dirroot/local/satsmail/tests/upgrade_test.xml");

        // Set message processor settings.

        set_config('message_provider_local_satsmail_mail_enabled', 'popup,email', 'message');
        set_config('popup_provider_local_satsmail_mail_locked', '0', 'message');
        ...
        $labelid1 = $DB->insert_record('local_satsmail_labels', ['userid' => $user1->id, 'name' => 'Label 1']);
        $labelid2 = $DB->insert_record('local_satsmail_labels', ['userid' => $user2->id, 'name' => 'Label 2']);
        $labelid3 = $DB->insert_record('local_satsmail_labels', ['userid' => $user3->id, 'name' => 'Label 3']);
        $labelid4 = $DB->insert_record('local_satsmail_labels', ['userid' => $user4->id, 'name' => 'Label 4']);
        $messageid1 = $DB->insert_record('local_satsmail_messages', [
            'courseid' => $course1->id,
            'subject' => ' Subject    1  ',
        ...
        $DB->insert_record('local_satsmail_message_users', [
        ...
        $DB->insert_record('local_satsmail_message_labels', [
            'messageid' => $messageid1,
            'labelid' => $labelid1,
        ]);
        $DB->insert_record('local_satsmail_message_labels', [
        ...
        $messageid2 = $DB->insert_record('local_satsmail_messages', [
            'courseid' => $course1->id,
            'subject' => ' Subject    2  ',
        ...
        $DB->insert_record('local_satsmail_message_refs', [
        ...
        $messageid3 = $DB->insert_record('local_satsmail_messages', [
            'courseid' => $course2->id,
            'subject' => ' Subject    3  ',
        ...
        set_config('version', 2013121100, 'local_satsmail');
        xmldb_local_satsmail_upgrade(2013121100);

        // Check upgraded schema.

        $xmldbfile = new \xmldb_file("$CFG->dirroot/local/satsmail/db/install.xml");
        $xmldbfile->loadXMLStructure();
        $dbman->check_database_schema($xmldbfile->getStructure());

        // Check web message processor is disabled.

        self::assertEquals('email', get_config('message', 'message_provider_local_satsmail_mail_enabled'));
        self::assertEquals('1', get_config('message', 'popup_provider_local_satsmail_mail_locked'));
