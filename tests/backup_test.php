...
/**
 * Tests for backup and restore of plugin data.
 *
 * @covers \backup_local_satsmail_plugin
 * @covers \restore_local_satsmail_plugin
 */
final class backup_test extends test\testcase {
    ...
    public function test_backup_and_restore(): void {
        ...
        set_config('enablebackup', 1, 'local_satsmail');
        ...
        foreach (array_keys(get_courses()) as $oldcourseid) {
            ...
            // Fetch old records.
            $oldlabels = $DB->get_records('local_satsmail_labels', [], 'userid, name');
            $oldmessages = $DB->get_records('local_satsmail_messages', ['courseid' => $oldcourseid], 'id');
            $oldmessagerefs = $DB->get_records_list('local_satsmail_message_refs', 'messageid', array_keys($oldmessages), 'id');
            $oldmessageusers = $DB->get_records('local_satsmail_message_users', ['courseid' => $oldcourseid], 'id');
            $oldmessagelabels = $DB->get_records('local_satsmail_message_labels', ['courseid' => $oldcourseid], 'id');
            $oldfiles = $fs->get_area_files(\context_course::instance($oldcourseid)->id, 'local_satsmail', 'message');
            ...
            $newlabels = $DB->get_records('local_satsmail_labels', [], 'userid, name');
            $newmessages = $DB->get_records('local_satsmail_messages', ['courseid' => $newcourseid], 'id');
            $newmessagerefs = $DB->get_records_list('local_satsmail_message_refs', 'messageid', array_keys($newmessages), 'id');
            $newmessageusers = $DB->get_records('local_satsmail_message_users', ['courseid' => $newcourseid], 'id');
            $newmessagelabels = $DB->get_records('local_satsmail_message_labels', ['courseid' => $newcourseid], 'id');
            $newfiles = $fs->get_area_files(\context_course::instance($newcourseid)->id, 'local_satsmail', 'message');
